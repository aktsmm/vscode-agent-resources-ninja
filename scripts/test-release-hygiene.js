...
function readZipEntries(filePath) {
  ...
  while (
    ...
    buffer.readUInt32LE(offset) === localHeader
  ) {
    ...
    if (flags & 0x08) {
      ...
      const descriptorOffset = buffer.indexOf(
...
test("vscodeignore excludes development and validation artifacts", () => {
  for (const pattern of [
    ...
    assert.ok(
      ...
      `Missing .vscodeignore pattern: ${pattern}`,
    );
  }
});

test("vscodeignore keeps the extension entrypoint while excluding sourcemaps", () => {
  assert.ok(
    hasLine(vscodeignore, "**/*.map"),
    "VSIX should not include sourcemaps",
  );
  for (const pattern of ["dist/**", "dist/", "dist/extension.js"]) {
    assert.ok(
      !hasLine(vscodeignore, pattern),
      `${pattern} would remove the package main entrypoint`,
    );
  }
});
...
test("gitattributes normalizes text without corrupting binary assets", () => {
  for (const pattern of [
    ...
    assert.ok(
      hasLine(gitattributes, pattern),
      `Missing .gitattributes pattern: ${pattern}`,
    );
  }
});

test("root has no stale backup files", () => {
  const backupFiles = listRootFiles().filter((name) =>
