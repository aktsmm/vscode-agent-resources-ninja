...
test("bug report links are generated by the shared helper", () => {
  ...
  assert.match(extensionSource, /await openBugReport\(issueTitle, issueBody\);/);
  assert.match(
...
test("esbuild log labels distinguish watch and one-shot builds", () => {
  ...
  assert.match(esbuildSource, /console\.log\(`\$\{logPrefix\} build started`\);/);
  assert.match(esbuildSource, /console\.log\(`\$\{logPrefix\} build finished`\);/);
});
...
test("built-in user resources remain openable and copyable", () => {
  ...
  for (const command of [
    ...
    assert.ok(
      ...
      `${command} should remain available for built-in resources`,
    );
  }
});
