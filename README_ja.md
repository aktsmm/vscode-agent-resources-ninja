...
- ツールバー: Resource Output / 新規作成 / 更新 / 設定
...
| `Agent Resources Ninja: Open Resource Output...`   | Command Palette から開く対象の managed scope を選択                           |
| `Agent Resources Ninja: Open Resource Folder`      | インストール済みリソースのフォルダを開く                                      |

view のツールバーと empty state のリンクは現在の view 文脈をそのまま使います。workspace view は workspace 出力を直接開き、User / Global Resource Home view は configured な global 出力を直接開きます。Command Palette のコマンドだけが明示選択用の QuickPick を出します。
...
| 表示とメンテナンス   | `outputFormat`, `refCatalogDirectory`, `refCatalogFormat`, `showBuiltInResources`, `remoteResourceViewMode`, `language`, `githubToken`                                                                                                                                                                     | 表示、探索、GitHub API 利用の調整                                   |
...
|  22  | `resourceNinja.outputFormat`                                    | `ref`                  | 出力形式（`ref` / `full` / `compact` / `legacy`）                                                                                                             |
|  23  | `resourceNinja.refCatalogDirectory`                             | `""`                   | `ref` 出力の catalog directory。空欄なら workspace target は `.github/resource-catalog`、global instruction file は同じ場所の `.catalog/resources` を自動使用 |
|  24  | `resourceNinja.refCatalogFormat`                                | `full`                 | `outputFormat = ref` のときに catalog 内で使う詳細形式（`full` / `compact` / `legacy`）                                                                     |
|  25  | `resourceNinja.showBuiltInResources`                            | `false`                | User / Global Resource Home に組み込みリソースを表示                                                                                                          |
|  26  | `resourceNinja.remoteResourceViewMode`                          | `repositoryFirst`      | Remote Resources の表示レイアウト                                                                                                                             |
|  27  | `resourceNinja.language`                                        | `auto`                 | UI 言語（auto / en / ja）                                                                                                                                     |
|  28  | `resourceNinja.githubToken`                                     | `""`                   | GitHub Token（API 制限緩和用）                                                                                                                                |
|  29  | `resourceNinja.instructionBlock.includeAgents`                  | `false`                | workspace の instruction block に `agent` を含める                                                                                                            |
|  30  | `resourceNinja.instructionBlock.includeInstructions`            | `false`                | workspace の instruction block に `instruction` を含める                                                                                                      |
|  31  | `resourceNinja.instructionBlock.globalHome.includeAgents`       | `inherit`              | Global Resource Home 向け agent 掲載ポリシーの上書き（`inherit` / `on` / `off`）                                                                              |
|  32  | `resourceNinja.instructionBlock.globalHome.includeInstructions` | `inherit`              | Global Resource Home 向け instruction 掲載ポリシーの上書き（`inherit` / `on` / `off`）                                                                        |
...
生成される instruction file には管理セクションが入ります。`coexistenceMode = auto` では `agent-ninja-START` / `agent-ninja-END`、`independent` モードでは従来の `resource-ninja-START` / `resource-ninja-END` を使います。手動編集は管理セクション外で行うか、ファイル全体を手動管理したい場合は自動更新を無効にしてください。生成済みセクションを安全にリセットしたいときは、`Resource NINJA: 管理マーカーブロックを削除` を実行してから `Update Instruction File` で再生成してください。

既定の `ref` 出力形式では、共有管理セクションを軽く保ちながら kind ごとの catalog へリンクします：
...
> See [Skills](.github/resource-catalog/skills.md) before working on tasks covered by these skills.

### Agents

> See [Agents](.github/resource-catalog/agents.md)

<!-- agent-ninja-END -->
```

詳細テーブルは catalog file 側に出力します。`full` / `compact` 形式の inline Description 列では、従来どおり `{description:80} | {whenToUse:80}`（合計最大160文字）を使います。
...
| ------------ | ----------------------------------------------------------------- | -------------------------------------------------- |
| **Ref**      | 軽量な参照ブロック + kind 別 catalog *(always-loaded file の既定)* | 別ファイル: `full` / `compact` / `legacy` から選択 |
...
`ref` フォーマットは instruction file を軽く保つための形式です。`coexistenceMode = auto` では **Skills** 参照だけに IMPORTANT を残し、`independent` モードでは skill-only block から skills catalog を参照します：
...
`resourceNinja.refCatalogDirectory` は空欄のままなら自動配置です。Workspace instruction target では workspace root 基準の `.github/resource-catalog`、Global Resource Home instruction target では instruction file を含む directory 基準の `.catalog/resources` を使います。たとえば `~/.copilot/copilot-instructions.md` なら、その周辺に `.catalog/resources/skills.md` を置きます。repo や global resource の配置を変えたい場合だけ directory を指定してください。catalog 内の詳細形式は `resourceNinja.refCatalogFormat` で切り替えます。`full` は source / remote metadata 付き、`compact` は path と短い説明、`legacy` はシンプルな resource / description table です。生成 catalog には `resource-ninja-catalog` marker を付与し、cleanup では marker 付きの生成 catalog だけを削除するため、同じ directory にある手書きファイルは保持されます。
...
| ------------------------------- | ------ | --------------------- | ---------- | ---------- | --------------------------------------------------- |
| [docx](../skills/docx/SKILL.md) | local  | `.github/skills/docx` | local      |            | Process Word documents (.docx). Use for .docx files |
```
...
`ref` を使う場合は、必要に応じて **Ref Catalog Output Directory** と **Ref Catalog Detail Format** も調整してください。
