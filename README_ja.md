...
|  16  | `resourceNinja.includeLocalResources`          | `false`                | workspace-wide fallback で検出した `SKILL.md` を Agent Skills index に含める          |
...
4. **workspace-wide fallback `SKILL.md` 検出** → `resourceNinja.includeLocalResources` が true の場合だけ Agent Skills index に追加
...
生成される instruction file には `resource-ninja-START` / `resource-ninja-END` で囲まれた管理セクションが入ります。手動編集は管理セクション外で行うか、ファイル全体を手動管理したい場合は自動更新を無効にしてください。
