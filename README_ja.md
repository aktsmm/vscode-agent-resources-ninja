...
|  27  | `resourceNinja.instructionBlock.includeAgents` | `true`                 | workspace の instruction block に `agent` を含める                                   |
...
既定では shared instruction block を意図的に薄く保ちます。`skill` は常に掲載、`agent` は既定で掲載、`instruction` は opt-in、`prompt` / `hook` / `mcp` / `plugin` / `cursor-rule` はネイティブ view に残します。Global Resource Home 向けの同期先は workspace の方針を継承するか、必要なときだけ個別に上書きできます。
