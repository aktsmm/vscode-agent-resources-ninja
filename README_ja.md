...
|  22  | `resourceNinja.useRefOutput`                                    | `true`                 | managed output を kind 別 catalog 参照で軽量化するか                                                                                                          |
|  23  | `resourceNinja.outputFormat`                                    | `full`                 | Ref 出力が off のときに使う inline 出力形式（`full` / `compact` / `legacy`）                                                                                  |
|  24  | `resourceNinja.refCatalogDirectory`                             | `""`                   | Ref 出力用の catalog directory。空欄なら workspace target は `.github/resource-catalog`、global instruction file は同じ場所の `.catalog/resources` を自動使用 |
|  25  | `resourceNinja.refCatalogFormat`                                | `full`                 | Ref 出力が on のときに catalog 内で使う詳細形式（`full` / `compact` / `legacy`）                                                                              |
|  26  | `resourceNinja.showBuiltInResources`                            | `false`                | User / Global Resource Home に組み込みリソースを表示                                                                                                          |
...
|  29  | `resourceNinja.githubToken`                                     | `""`                   | GitHub Token（API 制限緩和用）                                                                                                                                |
...
`useRefOutput`、`outputFormat`、`refCatalogDirectory`、`refCatalogFormat` など出力関連の設定変更は、`autoUpdateInstruction` が off でも managed output を即時再生成します。この設定は resource 変更時の自動同期だけを止めます。
...
既定の Ref 出力モードでは、共有管理セクションを軽く保ちながら kind ごとの catalog へリンクします：
...
| モード / 形式 | instruction file                                                   | catalog file (`refCatalogFormat`)                  |
| ------------- | ------------------------------------------------------------------ | -------------------------------------------------- |
| **Ref on**    | 軽量な参照ブロック + kind 別 catalog _(always-loaded file の既定)_ | 別ファイル: `full` / `compact` / `legacy` から選択 |
...
Ref 出力を使う場合は、必要に応じて **Ref Catalog Output Directory** と **Ref Catalog Detail Format** も調整してください。
