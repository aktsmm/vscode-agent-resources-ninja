...
| `Agent Resources Ninja: Update Resource Output`    | 現在のスコープの生成リソース出力を手動再生成                                  |
...
|  22  | `resourceNinja.useRefOutput`                                    | `true`                 | 生成リソース出力を kind 別 catalog 参照で軽量化するか                                  |
|  23  | `resourceNinja.outputFormat`                                    | `full`                 | Ref 出力が off のときに使う inline 出力形式（`full` / `compact` / `legacy`）           |
|  24  | `resourceNinja.refCatalogFormat`                                | `full`                 | Ref 出力が on のときに README index 内で使う詳細形式（`full` / `compact` / `legacy`）  |
