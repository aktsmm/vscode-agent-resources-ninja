...
- instruction file へ自動同期（`resourceNinja.includeLocalResources` 設定で制御）
- 手動での登録/解除コマンド
...
プリセットインデックスには、公式・キュレーション・コミュニティの各ソースから skills、agents、prompts、instructions、hooks が初期状態で含まれます。
...
- instruction file に登録可能なローカルリソース
...
- 選択中の preset（`~/.copilot`, `~/.claude`, `~/.agents`）配下の Global Resource Home リソース
...
- 組み込みリソースは読み取り専用で、インストール先にはなりません
...
| `Agent Resources Ninja: Create New Resource`       | ローカルの skill、agent、prompt、instruction、hook を作成        |
...
5. 完了！Workspace へのインストールは instruction file を自動更新できます
```
...
| Instruction sync     | `autoUpdateInstruction`, `instructionFile`, `customInstructionPath`, `includeLocalResources` | 任意の Agent Skills index 生成                                      |
...
|  13  | `resourceNinja.autoUpdateInstruction`          | `true`                 | skill 変更後に Agent Skills index を自動更新                                       |
|  14  | `resourceNinja.instructionFile`                | `AGENTS.md`            | Agent Skills index の同期先 _(要: Auto Update)_                                    |
|  15  | `resourceNinja.customInstructionPath`          | `""`                   | カスタム Agent Skills index パス _(instructionFile が 'custom' の時のみ)_          |
|  16  | `resourceNinja.includeLocalResources`          | `true`                 | ローカル `SKILL.md` を Agent Skills index に含める                                 |
|  17  | `resourceNinja.autoUpdateResourcesOnUpgrade`   | `prompt`               | 拡張機能アップグレード時にインストール済みリソースを更新                           |
|  21  | `resourceNinja.outputFormat`                   | `full`                 | 出力形式（full / compact / legacy）                                                |
|  22  | `resourceNinja.showBuiltInResources`           | `false`                | User / Global Resource Home に組み込みリソースを表示                               |
|  23  | `resourceNinja.remoteResourceViewMode`         | `repositoryFirst`      | Remote Resources の表示レイアウト                                                  |
|  24  | `resourceNinja.language`                       | `auto`                 | UI 言語（auto / en / ja）                                                          |
|  25  | `resourceNinja.githubToken`                    | `""`                   | GitHub Token（API 制限緩和用）                                                     |
...
3. **ローカル `SKILL.md` 検出** → workspace skills index に追加（`resourceNinja.includeLocalResources` が true の場合）
4. **登録/解除コマンド** → ローカル workspace skill の手動制御

agents、prompts、instructions、hooks はネイティブパスにインストールされ、各リソースビューで管理します。生成される Agent Skills index にはコピーされません。
