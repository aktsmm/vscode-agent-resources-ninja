...
| Shared caches           | `useSharedSourcesManifest`, `useSharedResourceIndex`, `staleSourceIndexUpdateMode`                                                                                                                                                                                                                         | Cross-extension SSOT and source index freshness behavior                  |
...
An explicit **Update Index** force-scans every configured source, oldest first, so a run cut short by a rate limit spends its budget on the sources that are furthest behind. Progress advances after each source finishes, and each result is logged as `OK`, `FAILED`, or `SKIPPED` in the **Agent Resources Ninja** Output Channel. Existing entries are preserved for failed sources; a GitHub rate-limit failure stops the remaining requests, reports them as not attempted, and schedules an automatic resume once the limit resets. The notification and `#updateResourceIndex` tool return one localized summary instead of reporting a partial update as full success. From that summary, **Configure GitHub Authentication** opens the relevant setting directly without showing a second error dialog.

Six safeguards keep a refresh from quietly damaging the index:
...
- **Automatic resume** - When a rate limit stops a refresh, the source that hit the limit and the ones never attempted are recorded and retried automatically once the window resets. The deadline comes from `Retry-After`, then `x-ratelimit-reset`, then a one-minute minimum, and nothing retries before it. Progress appears in the status bar, the result is reported either way, and a resume that is rate-limited again stops instead of chaining retries.
- **Honest freshness** - A source counts as fresh only when this machine actually scanned it. The bundled catalog's publish date is not treated as a scan time, so a source you have never indexed is reported as stale instead of hiding behind the catalog date. Each timestamp also records which extension wrote it: a timestamp written by the sibling extension is not counted as this extension's own scan, and the shared scan cache is consulted instead, so with `useSharedResourceIndex` enabled a recent scan by the sibling is not immediately repeated. An explicit **Update Index** always rescans.
...
Optional shared metadata is available through `resourceNinja.useSharedSourcesManifest` and `resourceNinja.useSharedResourceIndex` when you want both extensions to reuse the same remote source list and cache. Both are off by default, and both files live in a directory any tool on the machine can write to, so they are read as untrusted input and never as the whole truth:
