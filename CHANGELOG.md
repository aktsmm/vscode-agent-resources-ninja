...
- 🔗 **A Branch Name Could Break the Links We Show** - The URL builder behind resource previews and "open on GitHub" inserted the branch into the path without escaping it, so a branch containing `#` or `?` produced a link that pointed at the repository root instead of the resource. The branch is now escaped per path segment, exactly like the raw-content and API URLs already were, so an ordinary name such as `feature/x` keeps working / リソースのプレビューや GitHub で開く導線が使う URL 生成は、branch をエスケープせずパスへ入れていたため、`#` や `?` を含む branch ではリソースではなくリポジトリのトップを指すリンクになっていました。raw コンテンツ URL や API URL と同じく、branch をパスセグメント単位でエスケープします。`feature/x` のような通常の branch 名はこれまでどおり動作します。

- 🧭 **An Escalated Request Could Carry Extra Query Parameters** - When public raw content answered 404, the retry rebuilt a GitHub Contents API URL and copied the branch segment straight into `?ref=`. A raw URL that had reached that point unescaped could therefore add its own parameters to the API request. The ref is now decoded once and re-encoded for the query, so an already-escaped ref round-trips unchanged and an unescaped one can no longer alter the request / 公開 raw コンテンツが 404 を返したときの再試行は、GitHub Contents API の URL を組み立て直し、branch セグメントをそのまま `?ref=` へ入れていました。そのため未エスケープのまま届いた raw URL が API リクエストへ自分のパラメータを足せました。ref は一度デコードしてから query 用に再エンコードします。エスケープ済みの ref はそのまま往復し、未エスケープの ref はリクエストを変更できません。

- 🤝 **We Could Write a Shared File We Would Then Refuse** - The shared sources manifest was capped at 1 MB on read while the sibling extension allows 2 MB, so a file it wrote legitimately could be unreadable here — and a file we cannot read is one we refuse to overwrite, which stops sharing for good. The cap now matches the sibling. The writer also merges the entries already on disk with our own without re-checking the limits the reader enforces, so it could publish a file our own next read would reject. The merged result is now serialized once, checked against the same entry and byte limits used on read, and refused instead of written when it exceeds them / 共有 sources マニフェストは読み取り側を 1 MB で打ち切っていましたが、姉妹拡張は 2 MB まで許容するため、相手が正当に書いたファイルをこちらだけ読めない状態がありました。読めないファイルには上書きしない設計のため、その時点で共有が恒久的に止まります。上限を姉妹拡張と揃えました。書き込み側も、ディスク上のエントリと自分のエントリを merge する際に読み取り側の上限を再検査していなかったため、次の読み取りで自分が拒否するファイルを公開し得ました。merge 結果は 1 度だけ直列化し、読み取りと同じ件数・バイト上限で検査して、超過する場合は書かずに拒否します。

- 📏 **Two More Shared Files Could Be Written Past Their Own Read Limit** - The shared resource index enforced its resource and metadata counts on both sides but its 32 MB size only on read, and the rate-limit resume record enforced its 64 KB size only on read while the record it accepts can be larger than that. Either file could therefore be published in a state its own reader discards, which pauses sharing in one case and silently strands every deferred source in the other. Both writers now serialize once and check the bytes they are about to write against the same limit the reader applies, and a resume record that cannot be written is discarded rather than left behind, so a refusal means no resume instead of resuming an older set of sources / 共有リソースインデックスは件数の上限を読み書き両方で見ていましたが、32 MB のサイズ上限は読み取り側だけでした。rate limit の再開レコードも 64 KB の上限が読み取り側だけで、受理できるレコードはそれを超え得ました。どちらも、自分の読み取りが捨てる状態のまま公開でき、前者では共有が止まり、後者では延期した source が痕跡なく再開されなくなります。両方の書き込み側で、書こうとしている payload を 1 度だけ直列化し、読み取りと同じ上限で検査します。書けなかった再開レコードは残さず破棄するため、拒否は「再開しない」を意味し、古い source 集合を再開してしまうことはありません。

- 🔤 **A `#` or a Space in a File Name Broke Its URL** - Resource paths come from the repository tree, where `#` and spaces are legal, but the raw-content and repository URLs inserted them without escaping. A `#` ended the URL early, so the download or preview silently addressed a different file. Every URL composed from a repository path now escapes it per segment, while a path taken back out of an existing URL is left alone because it is already encoded, and a static check fails the build if a new raw URL skips the escape / リソースの path はリポジトリのツリー由来で、`#` や空白も正当なファイル名ですが、raw コンテンツ URL とリポジトリ URL はそれをエスケープせず埋め込んでいました。`#` は URL をそこで終わらせるため、ダウンロードやプレビューが黙って別のファイルを指していました。リポジトリの path から組み立てる URL はすべてセグメント単位でエスケープします。既存 URL から取り出した path は既にエンコード済みのためそのまま扱い、新しい raw URL がエスケープを飛ばした場合は静的チェックでビルドを失敗させます。
...
- 🧯 **The Shared Cache Deleted What the Other Extension Owned** - The shared sources manifest and the shared resource index are enabled by two independent settings, so turning on only the shared index left the sibling extension's sources absent from this extension's list. Every write then rebuilt the file from this extension's own view, deleting the sibling's resources and scan records, and the scan-metadata update runs after every source scan. Ownership is now read from each source's scan record: entries this extension owns are rewritten as before, and anything it does not own is kept because it cannot be rebuilt from here. A resource already loaded into the runtime index is written once rather than duplicated on every save / 共有 sources マニフェストと共有リソースインデックスは独立した 2 つの設定で有効化するため、共有インデックスだけを有効にすると、姉妹拡張の source がこの拡張の一覧に存在しない状態になります。その状態では書き込みのたびにこの拡張自身の内容からファイルを作り直し、姉妹拡張のリソースと scan 記録を削除していました。scan メタデータの更新は source をスキャンするたびに走るため、頻度も高い経路でした。所有権を各 source の scan 記録から判定するようにし、この拡張が所有するエントリは従来どおり書き直し、所有していないものはここから再生成できないため保持します。ランタイムのインデックスへ読み込み済みのリソースは、保存のたびに増殖せず 1 件だけ書き込みます。

- 🧱 **A Corrupt Shared Index Was Overwritten With Only Our Own Data** - The shared resource index got the same treatment the shared sources manifest already had: a file that could not be read was indistinguishable from one that did not exist, so both the periodic sync and the scan-metadata update rebuilt it from this extension's own view and wrote it back, discarding every resource and scan record only the sibling extension held. Scan metadata is written after every source scan, so this ran often. Both paths now tell the two cases apart and refuse to write over a file they could not read, leaving it exactly as it is / 共有リソースインデックスにも、共有 sources マニフェストで既に対処済みだったのと同じ問題が残っていました。読み取れないファイルと存在しないファイルを区別できないため、定期同期と scan メタデータ更新の両方がこの拡張自身の内容から作り直して書き戻し、姉妹拡張だけが持つリソースと scan 記録をすべて捨てていました。scan メタデータの更新は source をスキャンするたびに走るため、頻度も高い経路でした。両経路で 2 つの状態を区別し、読み取れなかったファイルには書き込まず、そのまま残すようにしました。

- 🔔 **Sharing Could Stop Working Without Saying So** - Refusing to write over an unreadable shared file is the right call, but it means syncing stops for good, and the only trace was a line in the output channel. Sharing looked healthy while it had silently stopped. Each shared file now reports the pause once, with actions to open the coexistence status or the log. The same reason stays quiet on later saves, a different reason speaks again, and the two shared files are tracked separately so one recovering does not silence the other / 読み取れない共有ファイルに上書きしないのは正しい判断ですが、その結果として同期は恒久的に停止し、痕跡は出力チャネルの 1 行だけでした。共有は正常に見えたまま、実際には止まっていました。共有ファイルごとに、停止したことを 1 回だけ通知し、共存ステータスまたはログを開くアクションを添えるようにしました。同じ理由では以降の保存で沈黙し、理由が変われば再度通知します。2 つの共有ファイルは別々に記録するため、片方が回復してももう片方の通知を消しません。

- 📂 **Your Own Sources Could Disappear When Sharing Was Turned On** - The shared sources manifest was treated as the complete truth about which sources exist, so any source it did not list was removed from the runtime index — and because that result is written straight back to the local index cache, the removal became permanent locally. Sharing is off by default, so every source added before turning it on was missing from that file, and enabling the setting on a machine where the sibling extension had already written it deleted them all. A source is now only removed on the strength of the shared file once this extension's own sources have actually reached it; until then the file's silence says nothing about them. An entry that is physically present but failed validation also counts as unusable rather than deleted, so a source is no longer lost just because its stored entry could not be read / 共有 sources マニフェストを「どの source が存在するかの完全な正」として扱っていたため、そこに載っていない source はランタイムのインデックスから取り除かれ、その結果がローカルのインデックスキャッシュへそのまま書き戻されるため、削除がローカルで恒久化していました。共有は既定で無効なので、有効化前に追加した source はこのファイルに載っておらず、姉妹拡張が先にファイルを書いていた端末で設定を有効にすると、それらがすべて消えていました。共有ファイルを根拠に source を削除するのは、この拡張自身の source が実際にそのファイルへ反映されて以降に限るようにしました。それまでは、ファイルに載っていないことは何の情報でもありません。物理的に存在するが検証に落ちたエントリも、削除ではなく「使用不可」として扱うため、保存されたエントリが読めないだけで source を失うことはなくなりました。

- 🏷️ **A Rescan Kept Someone Else's Name on the Timestamp** - Re-adding a repository that was already registered refreshed its index time but left the recorded writer untouched, so a source the sibling extension had indexed still claimed the sibling as its indexer after this extension rescanned it. Freshness ignores another extension's timestamp as evidence of its own scan, which meant the rescan did not count and the source stayed eligible for another refresh. The rescan now records this extension as the indexer, and a static guard fails the build if any future scan path writes an index time without recording who wrote it / 登録済みリポジトリを再度追加すると、インデックス時刻は更新される一方で記録された書き込み元はそのままだったため、姉妹拡張がインデックスした source は、この拡張が再スキャンした後も姉妹拡張がインデックス元であると主張し続けていました。鮮度判定は他拡張の時刻を自分の scan 実績として扱わないため、この再スキャンは実績として数えられず、その source は更新対象のまま残っていました。再スキャン時にこの拡張をインデックス元として記録するようにし、今後どこかの scan 経路がインデックス時刻だけを書いて記録元を残さない場合はビルドが失敗する静的ガードを追加しました。

- 🤝 **Sources Known Only to the Sibling Extension Were Lost** - The shared sources manifest is written by both this extension and the skill-only sibling extension, but a file this extension failed to read was indistinguishable from one that was not there at all. Both cases returned "nothing", and the caller answered by writing its own source list over the top, so every source only the sibling knew about disappeared. Reading now reports `missing`, `valid` or `rejected` separately, and the bootstrap that replaces the file only ever runs when the file is genuinely absent. A second path did the same damage without a corrupt file involved: the periodic sync wrote the in-memory source list wholesale. Every write now re-reads the file under the existing cross-process lock and merges into it, keying on source id — this extension's values win on the fields it owns, entries and fields written by anyone else survive untouched, and an entry is only removed when the view being written is still current. A source that this extension cannot validate is never used at runtime, but it is still carried through a rewrite verbatim rather than dropped / 共有 sources マニフェストはこの拡張と skill 専用の姉妹拡張の両方が書き込みますが、読み取りに失敗したファイルと最初から存在しないファイルを区別できていませんでした。どちらも「無い」として扱われ、呼び出し側が自分の source 一覧で上書きするため、姉妹拡張だけが知っている source がすべて消えていました。読み取りは `missing` / `valid` / `rejected` を区別し、ファイルを置き換える bootstrap は本当にファイルが存在しないときだけ実行するようにしました。破損ファイルが無くても同じ喪失を起こす経路がもう 1 つあり、定期同期がメモリ上の source 一覧をそのまま書き戻していました。書き込みは既存のプロセス間ロックの下でファイルを読み直し、source id を key に merge します。この拡張が所有するフィールドは自分の値が優先され、他者が書いたエントリとフィールドはそのまま残り、削除は書き込み時点の内容が最新である場合に限ります。検証できない source は実行時には一切使いませんが、破棄せずそのまま書き戻します。

- 🚫 **A Reader Moved Aside Files It Could Not Parse** - When the shared sources manifest or the shared resource index could not be parsed, the reader renamed it to `<file>.broken-<timestamp>`. That accumulated unbounded leftovers in a directory shared with another extension, and because the rename was never re-checked it could move aside a file the other process had just repaired. Readers no longer modify anything: an unusable file is reported and left exactly where it is. Files left behind by earlier versions are not touched, since deleting by pattern in a shared directory would also catch the sibling's temporary files / 共有 sources マニフェストと共有リソースインデックスは、parse に失敗すると読み取り側が `<file>.broken-<timestamp>` へリネームしていました。別拡張と共有するディレクトリに際限なくゴミが溜まるうえ、リネーム前の再確認が無いため、他プロセスが直した直後の正常なファイルを退避させ得ました。読み取り側は一切変更を加えず、使えないファイルは報告するだけでその場に残すようにしました。旧バージョンが残したファイルには触れません。共有ディレクトリでパターン削除を行うと姉妹拡張の一時ファイルまで巻き込むためです。

- 🛡️ **Shared Files Were Parsed Without Limits or Validation** - Any tool on the machine can write to the shared directory, but `sources.json` and `index.json` were read and parsed with no size limit, and a source entry was only checked for its numeric repository id and its scanner. Everything else — including the URL and the include/exclude path prefixes that decide what gets downloaded — was taken as written. Both files are now sized through the open file handle before being read, so the check cannot be raced by a rewrite, and an oversized file is rejected whole rather than truncated. Each entry is validated field by field: the id charset and length, a plain `https://github.com/<owner>/<repo>` URL, and path prefixes that must be relative, free of `..` segments and not absolute or UNC. An entry that fails is reported and ignored, and keys that would rewrite an object's prototype never reach the runtime / 共有ディレクトリは端末上のどのツールからも書き込めますが、`sources.json` と `index.json` はサイズ上限なしで読み込み・parse されており、source エントリの検証もリポジトリ数値 id と scanner だけでした。ダウンロード先を決める URL や include / exclude のパス接頭辞を含め、それ以外は書かれたまま採用していました。両ファイルとも読み込み前に開いたファイルハンドル経由でサイズを確認するようにし、stat と read の間の書き換えで検査を回避されないようにしたうえで、上限超過は切り詰めずファイル全体を拒否します。各エントリは項目ごとに検証します。id の文字種と長さ、`https://github.com/<owner>/<repo>` 形式のみの URL、相対で `..` を含まず絶対パスや UNC でないパス接頭辞が対象です。検証に落ちたエントリは報告のうえ無視し、オブジェクトのプロトタイプを書き換えるキーは実行経路へ渡しません。

### Changed

- 🕓 **A Source Nobody Ever Scanned No Longer Looks Fresh** - Per-source freshness fell back to the bundled catalog's publication date when a source had no scan time of its own. That date says when the catalog was published, not when anything was scanned, so a source that had never been indexed on this machine reported itself up to date and was never offered for a refresh. The fallback is gone: a source is only as fresh as the last scan of that source. Because both extensions write scan times into the same shared file, each timestamp now also records which extension wrote it, and a timestamp stamped by the other extension is not treated as this extension's own scan evidence — it falls back to the shared index, so work the sibling already did is not repeated. Timestamps written before this change carry no attribution and continue to count as this extension's own, so existing sources do not all turn stale at once. Sources that were previously masked as fresh may now appear in the stale-source prompt, which stays capped and follows the existing `resourceNinja.staleSourceIndexUpdateMode` setting (`prompt` by default) / source 単位の鮮度判定は、その source 自身の scan 時刻が無いとき同梱カタログの発行日にフォールバックしていました。この日付はカタログの発行時期であって scan 時刻ではないため、この端末で一度もインデックスしていない source が最新として報告され、更新候補にも挙がりませんでした。フォールバックを廃止し、source の鮮度はその source の最終 scan だけで判断するようにしました。両拡張が同じ共有ファイルへ scan 時刻を書き込むため、各時刻に「どの拡張が書いたか」を併せて記録し、他拡張が付けた時刻はこの拡張自身の scan 実績として扱いません。その場合は共有インデックス側へフォールバックするので、姉妹拡張が済ませた作業をやり直すことはありません。この変更より前に書かれた時刻は帰属情報を持たないため引き続き自拡張のものとして扱い、既存 source が一斉に stale 化しないようにしています。これまで新鮮と誤判定されていた source が stale 一覧に現れることがありますが、件数の上限と既存の `resourceNinja.staleSourceIndexUpdateMode` 設定（既定は `prompt`）はそのままです。

## [0.2.49] - 2026-08-17

### Added

- ⏳ **Automatic Resume After a GitHub Rate Limit** - A refresh stopped by a rate limit used to abandon every source it had not reached, so the same work had to be started again by hand. The run now records the source that hit the limit together with the ones it never attempted, and resumes them automatically once the window resets. The deadline follows GitHub's documented order — `retry-after`, then `x-ratelimit-reset` once the window is exhausted, then a conservative one-minute minimum — so a secondary rate limit that sends only `retry-after` is no longer left without one. Nothing retries before that deadline, a small margin and jitter keep a clock skew from firing early, and a resume that is rate-limited again records the new deadline instead of chaining retries. Reading a pending record always re-arms the timer, so a window opened before the deadline cannot leave it stranded, and the claim is taken under the existing cross-process lock so two windows reaching the deadline together cannot both resume. The warning now states when the retry will happen, and adds the 60-versus-5,000 requests-per-hour difference when no GitHub credential is configured. The resume reports status-bar progress while it runs and closes the loop it promised: it says how many deferred sources it recovered, or that it was rate-limited again and will not retry automatically. A manual update that cannot start because another update is already running now says so instead of doing nothing. The resume record shares a directory with the skill-only sibling extension, so it is read as untrusted input: an oversized file is rejected before it is parsed, the source list is bounded and deduplicated, and over-long claim fields are dropped / rate limit で中断した更新は、到達しなかった source をすべて捨てていたため、同じ作業を手動でやり直す必要がありました。制限に当たった source と未試行の source を記録し、制限解除後に自動で再開するようにしました。解除時刻は GitHub の文書どおり `retry-after`、枠を使い切った場合の `x-ratelimit-reset`、いずれも無い場合は 60 秒の最小待機、の順で決めます。`retry-after` しか返さない secondary rate limit でも期限が決まります。期限前には再試行せず、わずかなマージンとジッターで時計ずれによる早すぎる発火を防ぎ、再開が再び制限された場合は新しい期限を記録するだけで連鎖しません。保留中のレコードを読んだときは必ずタイマーを張り直すため、期限前に開いたウィンドウがレコードを取り残すことはありません。claim は既存のクロスプロセスロック内で行うため、同時に期限へ達した 2 つのウィンドウが両方とも再開することはありません。警告には再開予定時刻を表示し、GitHub の資格情報が未設定のときは 60 リクエスト/時間と 5,000 リクエスト/時間の差も併記します。再開処理は実行中はステータスバーに進捗を出し、約束した結末まで伝えます。保留していたソースを何件更新できたか、または再びレート制限に達して自動再試行を行わないことを通知します。別の更新が実行中で手動更新を開始できない場合も、無反応ではなくその旨を表示します。再開レコードは skill 専用の姉妹拡張と同じディレクトリを共有するため untrusted input として読み、サイズ超過のファイルは parse 前に拒否し、source 一覧は上限付きで重複排除し、長すぎる claim 値は破棄します。

### Changed

- 🕓 **Oldest Sources Refresh First** - The full index update walked sources in stored order, so a run cut short by a rate limit always spent its budget on the same head of the list and the tail could stay stale indefinitely. It now walks them oldest first, with never-indexed sources ahead of everything else and ties keeping their stored order, and the not-attempted list is derived from that same order. Membership is unchanged; only the order is. The startup stale refresh keeps its existing cursor rotation / フルインデックス更新は保存順に source を走査していたため、rate limit で中断した実行は毎回同じ先頭部分に予算を使い、末尾がいつまでも古いままになり得ました。未インデックスの source を先頭に、同着は保存順を保ったまま、古い順に走査するようにし、未試行リストも同じ順序から導出します。対象そのものは変わらず、順序だけが変わります。起動時の stale 更新は既存のカーソル回転を維持します。

### Fixed

- 🧹 **Empty Retired Sources in Remote Resources** - A preset source that was retired or consolidated in a newer resource index stayed in the cached index forever, because the bundled-index merge only added and updated entries. Such sources kept showing in Remote Resources as a `0 resources` row (for example `Goose Skills (Official / AAIF)` and `GitHub Copilot for Azure Skills (Official)`). The merge now also drops a cached source that is absent from the bundled index and has no resources and no bundles left. A retired source whose successor is bundled is additionally consolidated, but only for the resources the successor already ships — an entry that exists nowhere else is kept, because its path belongs to the old repository and cannot be served from the new one. User-added sources and any source that still has indexed content are never removed / 新しい resource index で退役または統合された preset source が cached index に残り続けていました。bundled index の merge が追加と更新しか行わなかったためです。該当 source は Remote Resources に `0 resources` の行として表示され続けていました（例: `Goose Skills (Official / AAIF)`、`GitHub Copilot for Azure Skills (Official)`）。merge 時に、bundled index に存在せずリソースも bundle も残っていない cached source を取り除くようにしました。後継 source が同梱されている退役 source は統合しますが、後継側に同じリソースが揃っているものだけが対象です。他に存在しないリソースは、path が旧リポジトリのもので後継からは配信できないため残します。ユーザー追加 source と、まだ内容が残っている source は削除しません。

- 🕒 **Index Freshness Reported Backwards** - The index carried one `lastUpdated` field with two conflicting meanings: a source scan wrote today's date into it, while the bundled-catalog merge overwrote it with the catalog publication date on the next activation. An index scanned minutes ago could therefore report itself weeks old, which drove the "index is out of date" warning. Scans now record a separate `lastScannedAt` instant and `lastUpdated` means the bundled catalog date only. The index-wide warning reads the scan timestamp first and falls back to `lastUpdated` for indexes written by earlier versions, while per-source staleness deliberately keeps using each source's own timestamp so that scanning one source cannot mark every other source fresh. Removing a source no longer counts as a scan, and the four scan paths now agree on one timestamp instead of mixing UTC and local dates / index は `lastUpdated` 1 つに 2 つの意味を持たせていました。source の scan が当日日付を書き込む一方、同梱カタログの merge が次回起動時にカタログ発行日で上書きしていました。このため直前に scan した index が数週間前として報告され、「インデックスが古い」警告の判定を狂わせていました。scan は独立した `lastScannedAt` を記録し、`lastUpdated` は同梱カタログの日付だけを表すようにしました。index 全体の警告は scan 時刻を優先して旧バージョンが書いた index では `lastUpdated` へフォールバックします。source 単位の鮮度判定は各 source 自身の時刻を使い続けます。1 つの source を scan しただけで他の source まで新鮮と判定させないためです。source の削除は scan として扱わなくなり、4 つの scan 経路は UTC と ローカル日付の混在をやめて同じ時刻表現に揃えました。

- 🔎 **Unexplained Empty Sources** - A source row with no indexed resources looked identical to a healthy one, so a retired preset and a source whose last scan came back empty were indistinguishable. Such a row now carries a warning icon and a tooltip stating when the source was last indexed, or that it has never been indexed on this machine, plus the hint to run Update Index. Sources that carry bundles are not flagged / インデックス済みリソースが無い source 行が正常な行と同じ見た目だったため、退役した preset と最後の scan が空で返った source を区別できませんでした。該当行に警告アイコンと、最終インデックス時刻（またはこの端末で未インデックスであること）と「インデックス更新」の案内を出す tooltip を付けました。bundle を持つ source は警告対象にしません。

## [0.2.48] - 2026-08-17

### Fixed

- 🔐 **Remembered GitHub SSO Rejections** - A credential rejected by organization SSO or classic-PAT policy is now remembered per repository owner for 10 minutes, so it is no longer re-sent on every request of the same scan and the 403-then-anonymous-retry cycle stops after the first rejection. The credential fallback walk now classifies and records its own responses, skips candidates already known to be rejected, and a plain permission failure never blocklists an owner. A blocked credential still escalates a raw 404 onto the API URL without its Authorization header, so the 404 keeps the meaning branch fallback depends on while a later credential can still authenticate. The record lives only in memory, is keyed by a token fingerprint that is never logged, and is cleared when an index update, source add, install, resource preview, GitHub search, credential reset, or SSO authorization starts, while shared helpers that run per source or per file deliberately keep it. Starting suppression writes exactly one output-channel line naming the owner and the rejection reason, so a later `404` stays traceable / organization SSO や classic PAT policy に拒否された credential を repository owner 単位で 10 分間記憶し、同じ scan の毎 request で再送しないようにしました。403 と匿名 retry の反復は最初の拒否で止まります。credential fallback walk は自身の response も分類して記録し、拒否済みの候補を飛ばします。単なる権限不足では owner をブロックしません。ブロック済み credential でも raw 404 の API URL への escalation は行い、Authorization だけを外すため、branch fallback が依存する 404 の意味を保ったまま後続の credential が認証できます。記録は memory 上のみで、log に出さない token fingerprint を key にし、index 更新、source 追加、install、resource preview、GitHub 検索、credential reset、SSO 認可の開始時に消去し、source や file ごとに実行される共通 helper では意図的に保持します。抑止開始時には owner と拒否理由を示す output channel の 1 行だけを記録し、後から `404` の原因を追えるようにしました。

- 🧭 **Root-Cause GitHub Failure Reasons** - When one credential is rejected for SSO and a later one only reports a rate limit, the authentication dialog, source-update failure reasons, and Language Model tool results now name the SSO rejection instead of the rate limit, using one shared reason mapping across every surface. The surfaced failure kind is unchanged, so rate-limit batch abort and 404 branch fallback keep behaving exactly as before. An SSO failure also offers a new **Open Organization SSO Authorization** action, built from the `X-GitHub-SSO` header and accepted only for an `https://github.com` organization SSO path. The pending authorization value is stored non-enumerably so serializing the error cannot leak it / ある credential が SSO で拒否され、後続の credential が rate limit だけを返した場合に、認証 dialog と source 更新の失敗理由が rate limit ではなく SSO 拒否を示すようにしました。表面化する failure kind は変更していないため、rate limit による batch 中断と 404 の branch fallback は従来どおり動作します。SSO 失敗時には新しい **organization の SSO 認可を開く** アクションを表示します。URL は `X-GitHub-SSO` header から生成し、`https://github.com` の organization SSO path のみを受け付けます。保留中の認可値は非列挙プロパティとして保持するため、error の serialize で漏れません。認証 dialog、source 更新の失敗理由、Language Model tool の結果は同じ理由 mapping を共有します。
